    ...
    internal static class UnsafeNativeMethods
    {
        ...
        internal static int rushquant_initialize(string pUsername, byte[] pKey)
        {
            if (__is64bit)
                ...
                internal static extern int rushquant_dispose32(IntPtr pointer);
                [DllImport("RushQuantClient64.dll", EntryPoint = "rushquant_dispose")]
                internal static extern int rushquant_dispose64(IntPtr pointer);
                internal static int rushquant_dispose(IntPtr pointer)
                {
                    if (__is64bit)
                        return rushquant_dispose64(pointer);
                    else
                        return rushquant_dispose32(pointer);
                }
